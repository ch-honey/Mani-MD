...
if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
  ...
  if (!sessionId || sessionId.toLowerCase() === 'enter your session') {
    console.log('Please add your session to SESSION_ID env !!');
  } else {
    const sessdata = sessionId.replace("𝗠𝗔𝗡𝗜𝗜-𝟬𝟱-𝗠𝗗~", '').trim();
    if (!sessdata || !sessdata.includes('#')) {
      console.log('SESSION_ID is invalid. Use the complete Mega session value, including the # key.');
    } else {
...
//======HEART REACTIONS =======
  ...
  app.get('/api/pair/status', (req, res) => {
    ...
    res.json({
      ...
      message: connected
        ? 'WhatsApp successfully connected hai.'
        : 'Bot WhatsApp se connect ho raha hai. Pairing code ke liye thori dair baad try karein.'
    });
  });
  ...
  app.post('/api/pair', async (req, res) => {
    try {
      if (connectionStatus === 'connected') {
        return res.status(409).json({ message: 'WhatsApp pehle se connected hai. Pairing code ki zaroorat nahi.' });
      }
      if (!activeConn || typeof activeConn.requestPairingCode !== 'function') {
        return res.status(503).json({ message: 'WhatsApp connection abhi ready nahi. 10 seconds baad dobara try karein.' });
      }
      const phoneNumber = String(req.body?.phone || '').replace(/\D/g, '');
      if (!/^\d{10,15}$/.test(phoneNumber)) {
        return res.status(400).json({ message: 'Valid WhatsApp number country code ke saath enter karein, example: 923001234567.' });
      }
      const now = Date.now();
      if (now - lastPairingAt < 30000 && lastPairingNumber === phoneNumber && lastPairingCode) {
        return res.json({ code: lastPairingCode, message: 'Aapka recent pairing code yahan hai. WhatsApp mein enter karein.' });
      }
      lastPairingAt = now;
      lastPairingNumber = phoneNumber;
      lastPairingCode = await activeConn.requestPairingCode(phoneNumber);
      return res.json({ code: lastPairingCode, message: 'Pairing code mil gaya. Isay WhatsApp → Linked devices mein enter karein.' });
    } catch (error) {
      console.error('PAIRING CODE ERROR:', error);
      return res.status(500).json({ message: 'Pairing code generate nahi ho saka. Number aur server logs check karein.' });
    }
  });
